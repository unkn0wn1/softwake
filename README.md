...
Speak into the default input; the HUD capsule particles should bloom with your voice. Wake-from-voice is not enabled yet: PCM is scored by `NullDetector` until sherpa-onnx KWS weights are installed under `$XDG_DATA_HOME/softwake/kws` (or `~/.local/share/softwake/kws`). See [ADR 0006](docs/ADR-0006-on-device-wake.md). Download is operator-consent only; weights are not vendored.
