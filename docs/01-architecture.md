...
| `softwake-tools` | Tool registry with safe, confirm, and deny metadata. Operator modes are Always allow, Ask, or Deny in `tools.json` version 2. Defaults: echo always allow, notify and email_send ask, shell deny ([ADR 0018](ADR-0018-tools-settings-shell.md)) |
...
| `softwake-policy` | Classifies tool names and connector pairs. Unknown subjects are denied. Overrides may only tighten. Operator Always allow is `effective_tool_decision`, not an override row ([ADR 0010](ADR-0010-policy-engine.md)) |
...
- Clients may send `wake` (`id` only). The daemon runs the wake-phrase path. Success broadcasts `state_changed`. Refusal is `kind: protocol` with the existing sentence. `wake_from_ui` still means leave hibernate into sleep. Generation stays 1.
...
- `notify` is confirm-gated in the registry. It appends one line to an in-memory sink after `confirm_tool`, or immediately when Tools Settings say Always allow. `shell` is confirm-gated in the registry and deny until Settings → Tools sets Ask or Always allow ([ADR 0018](ADR-0018-tools-settings-shell.md)).
...
Credentials and Settings Test live in `softwake-providers` ([ADR 0012](ADR-0012-model-providers.md)). Three kinds ship: xAI device-code OAuth, xAI API key, and OpenAI API key. Secrets prefer the OS keyring. The file under `$XDG_STATE_HOME/softwake/secrets.json` (mode `0600`, parent `0700` when this crate creates it) is a version-2 pointer with no secret fields, or an opt-in plaintext bag with a loud warning when the keyring is unavailable. Non-secret selection and the Test chat and voice/STT model caches are `$XDG_CONFIG_HOME/softwake/providers.json`. Both model dropdowns stay empty until Test succeeds. Default crate tests use `MockTransport` and do not open a socket. The `live-http` feature enables `ureq`. The window Settings panel talks to this crate through Tauri commands. IPC protocol generation stays 1. While awake, typed `ask` and `chat` on `softwaked demo` send the rendered context pack and the user line to the selected provider ([ADR 0013](ADR-0013-session-provider.md)). Tests use `MockTransport`. The daemon `live-http` feature performs the real call and is off by default; without it the demo returns a clear error and does not open a socket. `softwaked serve` accepts additive `ask` and `wake` messages on protocol generation 1. `softwaked ctl ask` and `ctl chat` send it. The assistant text comes back on status. A refusal is `chat_rejected`. The default build still does not call the provider.

## Policy

Tool and connector classification goes through `softwake-policy` ([ADR 0010](ADR-0010-policy-engine.md)). `PolicyEngine::evaluate` reads the static tool registry and the static connector registry. It does not copy those rows. An unknown tool name or connector pair is deny. A connector evaluation is confirm or deny. An override, when one is supplied, may only raise risk. The daemon's engine has an empty override map, so `evaluate` on a tool stays the registry floor.

`Hands::request` does not treat operator Always allow as an override row. While awake it calls `effective_tool_decision`: registry floor, then the Tools Settings grant, then a tighten-only soul request. A registry deny stays deny. Sleep and hibernate still refuse every tool before that decision. Default echo runs immediately. Default notify and email_send wait for `confirm_tool`. Default shell is denied. Ask stages one pending confirmation. Always allow runs without a prompt (shell still expands glossary aliases). An unknown tool name is still reported as unknown and does not read the permission map. Protocol generation stays 1.

## Memory

Long-term memory is a library boundary in `softwake-memory` ([ADR 0009](ADR-0009-long-term-memory.md)). `Memory` is the trait (`remember`, `recall`, `forget`). `MockMemory` is the default backend. It stores snippets on the value only after that value is enabled, and it does not open a socket or write a file. `FileMemory` is the opt-in file backend. It stays off until `open_enabled`, then writes `memory.json` under `$XDG_STATE_HOME/softwake` when that variable is set and non-blank, and under `~/.local/state/softwake` otherwise. The directory is created on the first successful write. The daemon calls `recall_for_prompt` when an awake ask builds its appendix. `softwake-session` does not depend on this crate. Soul rendering stays instruction-only. Honcho is not a dependency.
