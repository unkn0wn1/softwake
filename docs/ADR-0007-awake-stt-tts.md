...
- **Amended:** 2026-09-26. Half-duplex mute-while-speaking: while Eve TTS plays (and ~200ms grace), mic frames are dropped so speakers cannot self-trigger free speech / KWS / PTT. AEC deferred.
...
- **Playback.** The daemon writes a temp mp3 and tries `ffplay -nodisp -autoexit -loglevel quiet`, then `mpv --no-video --really-quiet`. A missing player is a HUD error. The player is **spawned and detached**: Softwake returns as soon as the child starts and does **not** `.wait()` on Eve finishing. A reaper joins with a timeout, kills the player if it is still running, and deletes the temp file. The product default is 60 s (`PLAYBACK_TIMEOUT`). Settings → General, `tts_playback_timeout_ms` in softwake.json, or `SOFTWAKE_TTS_PLAYBACK_TIMEOUT_MS` (env wins over the file) set it; values clamp to 30–300 s. The next speak uses the resolved deadline. An in-flight clip keeps the deadline it started with. `softwaked ctl reload-playback` confirms the running daemon. Spoken audio longer than the deadline can be cut even though the text reply is already complete and shown in full. Chat HTTP uses a separate 120 second budget ([ADR 0013](ADR-0013-session-provider.md)). A new ask interrupts the previous player when possible. The HUD bloom and UI stay responsive while she talks. From spawn until the player exits (plus ~200ms grace), Softwake **mutes mic input** (half-duplex): free-speech, KWS, and PTT frames are dropped so Eve over speakers cannot talk to herself. Skipped/failed TTS never leaves mute stuck. Acoustic echo cancellation is out of scope for v1.
- **Listen while awake (free speech).** While **awake**, capture running, PTT not armed, mic not muted for TTS, and outside a short post-ask cooldown (~2.5s to avoid Eve echo), an energy / silence gate (`EnergyUtterance`, default 2.0 s end-of-speech silence, configurable 0.5–4.0 s) collects an utterance from PipeWire PCM and runs the same STT→ask→Eve path as PTT. Settings → General, `free_speech_end_silence_ms`, and `SOFTWAKE_FREE_SPEECH_END_SILENCE_MS` (env wins) set that hangover; live reload does not rebuild wake-word spotting. Sleep→awake by voice still needs KWS weights. `Status.auto_listening` is additive and omitted when false. PTT always takes priority.
