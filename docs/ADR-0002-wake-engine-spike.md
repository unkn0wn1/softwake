...
This is a spike. It is not an on-device wake-word model. A later ADR will choose that engine. The PCM `WakeDetector` trait stays in place for it. `NullDetector` still never matches.
...
`PipeWireCapture` implements the capture trait and returns an error. The default `pipewire` feature compiles that stub and does not link a native audio library. The demo uses `MockAudioCapture`.
...
- Replacing the spike means implementing `WakeDetector` for the chosen engine and keeping the same `PhraseHit` to voice-state event mapping. The phrase table can remain as configuration after the engine changes.
- `softwaked demo` simulates voice by submitting phrase text. Capture still has to be running before that text is scored.
