...
- Good enough to try as the always-on gate for a short phrase list. Similar-sounding speech will false-accept sometimes. sherpa-onnx exposes a keyword score and a threshold; Softwake defaults are looser than sherpa stock: global threshold **0.15**, short single-word per-keyword `#0.10` (e.g. `sally`), score 1.0. Multi-word `hey <name>` stays on the global default and is usually easier to spot. Operators can override via `softwake.json` (`kws_threshold_milli` / `kws_short_threshold_milli`) or `SOFTWAKE_KWS_THRESHOLD` / `SOFTWAKE_KWS_SHORT_THRESHOLD`. At `-vv`, a probe stream at **0.05** logs near-miss keywords that did not reach the fire threshold (sherpa does not expose below-fire scores).
