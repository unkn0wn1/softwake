...
- TTS playback reaper defaults to 60 s (range 30–300 s). Settings → General writes `tts_playback_timeout_ms`. `SOFTWAKE_TTS_PLAYBACK_TIMEOUT_MS` wins. The next speak uses it. `softwaked ctl reload-playback` confirms the running daemon. The written reply is not cut. Half-duplex mute still covers the whole clip plus the short grace.
