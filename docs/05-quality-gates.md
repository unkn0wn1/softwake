...
Manual checklist. Run it after clone + soul templates. The typed demo (`softwaked demo`) enters awake in its own process. `softwaked ctl wake` is the serve path into awake. Default builds use mock capture and do not open a microphone. Native PipeWire is the `pipewire-native` / daemon `pipewire-capture` feature and CI does not enable it. Wake and sleep phrases in the demo are typed commands that feed the same state machine. The PCM detector still sees a silent frame on that path ([ADR 0006](ADR-0006-on-device-wake.md)).
...
| Confirm tool | Default shell is deny. Operator Ask waits for confirm. Operator Always allow runs without a prompt | Typed demo while awake: `tool notify hello` prints `waiting for confirm` and does not append; `confirm` appends `hello`; `cancel` appends nothing. `tool email_send ada@example.com hello a short note` stays empty until `confirm`, which appends one in-memory message. `tool shell` is denied until Tools Settings set ask (then it waits for confirm) or always allow (then it runs after glossary expand, with no prompt). Sleep or hibernate clears a pending confirmation. |
